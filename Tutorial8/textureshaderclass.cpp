...
bool TextureShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	error = wcscpy_s(vsFileName, 128, L"../DirectX11stuff/texture.vs");
	if (error != 0)
	{
		return false;
	}
	...
	error = wcscpy_s(psFileName, 128, L"../DirectX11stuff/texture.ps");
	if (error != 0)
	{
		return false;
	}

	// Initialize the vertex and pixe shaders
...
{
	// Shutdown the vertex and pixel shaders as well as the related obejcts
	ShutdownShader();
}
...
bool TextureShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFileName, WCHAR* psFileName)
{
	...
	if (FAILED(result))
	{
		...
		// If there was nothing in the error message, then the shader file could not be found
		else
		{
	...
	// Getting a count of the elements in the layout
	...
	// Create the context buffer pointer so the vertex shader constant buffer can be accessed from this class
											 ...
											 XMMATRIX projectionMatrix, ID3D11ShaderResourceView* texture)
{
	...
	// Lock the constant buffer so that it can be read to
